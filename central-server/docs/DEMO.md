...
## 시나리오
1. **정책 설정(관리자)**
   - `/settings` → 웹 대시보드에서 LLM 허용 채널, 자동 승인, 기본 모델, 프로바이더 승인을 설정
2. **프로바이더 등록**
   - A, B 가 `/provider-join` → (수동 승인 서버면) 관리자가 `/settings` 웹 대시보드에서 승인 → 토큰 발급
