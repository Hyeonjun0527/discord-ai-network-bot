...
class GuildHistoryBackfillService {
     ...
     *  3. 빈 본문 제외.
     *  4. [KnowledgeSafety.containsSensitiveMaterial] 매칭 라인은 색인 텍스트에서 제거(스크럽 카운트 증가).
    ...
    fun sanitizeMessages(
        ...
        for (msg in rawMessages) {
            ...
            val (cleanedLines, scrubbedHere) = scrubLines(content)
            scrubbed += scrubbedHere
            if (cleanedLines.isEmpty()) continue // 전부 민감 라인이라 색인할 본문이 없음
    ...
    /** 라인 단위로 민감정보를 제거한다. 살아남은 라인 목록과 제거된 민감 라인 수를 돌려준다(값은 절대 노출하지 않음). */
    private fun scrubLines(content: String): Pair<List<String>, Int> {
        var scrubbed = 0
