...
object ModelClassifier {
    ...
    fun maxBurden(modelNames: List<String>): String {
        val joined = modelNames.joinToString(" ").lowercase()
        return when {
            // 초대형·MoE → HEAVY
            listOf("70b", "65b", "72b", "405b", "large", "mixtral").any { it in joined } -> "HEAVY"
            // 대형(≥27B) → HEAVY. Gemma 27B 포함.
            listOf("27b", "30b", "32b", "34b").any { it in joined } -> "HEAVY"
            // 중형(9~14B)·코드 모델 → STANDARD. Gemma 9B/12B 포함.
            listOf("9b", "12b", "13b", "14b", "coder", "deepseek").any { it in joined } -> "STANDARD"
