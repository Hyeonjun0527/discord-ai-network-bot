...
class OpenAiConversationEmbeddingAdapter(
    ...
    private val usageObserver: ConversationEmbeddingUsageObserver = ConversationEmbeddingUsageObserver.NOOP,
    ...
    override fun embed(texts: List<String>): List<FloatArray> {
        ...
        val request =
            HttpRequest
                ...
                .build()
        runCatching { usageObserver.recordAttempt(model, requestBody.length) }
        val response = http.send(request, HttpResponse.BodyHandlers.ofString())
        check(response.statusCode() in 200..299) { "OpenAI embedding 호출 실패: HTTP ${response.statusCode()}" }
        val root = mapper.readTree(response.body())
        val promptTokens = root.path("usage").path("prompt_tokens").asInt(0)
        runCatching { usageObserver.record(model, promptTokens) }
