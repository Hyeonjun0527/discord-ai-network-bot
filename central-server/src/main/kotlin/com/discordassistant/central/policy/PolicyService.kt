...
class PolicyService(
    ...
    // ── 채널 정책 ───────────────────────────────────────────────────────
    @Transactional
    fun allowChannel(guildId: Long, channelId: Long, adminId: Long) {
    ...
    @Transactional
    fun denyChannel(guildId: Long, channelId: Long, adminId: Long) {
    ...
    // ── 역할 정책 ───────────────────────────────────────────────────────
    @Transactional
    fun setRolePolicy(guildId: Long, roleId: Long, maxBurden: ModelBurden, dailyLimit: Int, adminId: Long) {
    ...
    fun dailyLimit(guildId: Long, memberRoleIds: Collection<Long>, base: Int = 20): Int {
        val policies = roles.findByGuildId(guildId).filter { it.roleId in memberRoleIds }
        return policies.maxOfOrNull { it.dailyLimit } ?: base
    }

    /** 필요한 부담 수준을 이 멤버가 쓸 수 있는가(RESTRICTED 는 별도 정책 — 여기선 false). */
    fun isBurdenAllowed(memberMax: ModelBurden, required: ModelBurden): Boolean =
        required != ModelBurden.RESTRICTED && required.ordinal <= memberMax.ordinal
