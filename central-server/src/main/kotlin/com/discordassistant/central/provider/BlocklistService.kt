 ...
 */
@Service
class BlocklistService(private val audit: AuditLog) : BlocklistChecker {
    private val blocked = ConcurrentHashMap<Long, MutableSet<Long>>()

    override fun isBlocked(guildId: Long, userId: Long): Boolean =
        blocked[guildId]?.contains(userId) == true

    fun block(guildId: Long, userId: Long, adminId: Long) {
    ...
    fun unblock(guildId: Long, userId: Long, adminId: Long) {
