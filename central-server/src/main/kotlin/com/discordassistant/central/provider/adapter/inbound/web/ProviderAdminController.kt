...
class ProviderAdminController(
    ...
    /** 승인 대기·로스터 조회(관리 화면 13). 권한 없으면 ok=false + 빈 목록. */
    @PostMapping("/manage")
    fun manage(@RequestBody req: AdminActionRequest): ManageResponse {
        ...
        val roster = registration.providersInGuild(req.guildId)
            .map { ManageProviderDto(it, registration.stateOf(it, req.guildId)?.name ?: "UNKNOWN") }
        return ManageResponse(true, registration.pending(req.guildId), roster)
    }
}
