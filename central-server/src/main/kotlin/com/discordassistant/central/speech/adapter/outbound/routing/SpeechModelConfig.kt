 ...
 *
 * Luna 를 기본 후보로 두되 모델 ID·endpoint(타임아웃·재시도)를 **환경 설정**으로 분리한다.
 *
 ...
 *
 * endpoint(base-url)·api-key 는 routing 의 CloudLlm 구현(`central.cloud.openai-*`)이 SSOT 로 소유한다 — speech 는
...
class SpeechModelConfig(
    ...
    companion object {
        const val DEFAULT_SPEECH_MODEL = "gpt-5.6-luna"
    }
