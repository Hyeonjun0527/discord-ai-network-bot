...
class DashboardControllerTest
    ...
    constructor(
        ...
        fun `overview — 정책·요청 수 집계`() {
            ...
            requests.save(AiRequestEntity(requestId = "r1", guildId = 700, state = RequestState.COMPLETED, createdAt = Instant.EPOCH))
            requests.save(AiRequestEntity(requestId = "r2", guildId = 700, state = RequestState.REJECTED, createdAt = Instant.EPOCH))
        ...
        fun `requests — 공개 기본값은 프롬프트와 provider snowflake 를 노출하지 않는다`() {
            requests.save(
                AiRequestEntity(requestId = "r3", guildId = 701, state = RequestState.COMPLETED, providerId = 9, createdAt = Instant.EPOCH),
            )
