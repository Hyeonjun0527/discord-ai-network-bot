...
class ProviderOnboardingInstallTest {
    ...
    fun `macOS — brew + 바이너리 + 토큰·relay`() {
        ...
        assertTrue(s.contains("releases/latest/download/discord-ai-network-bot-macos"), s)
    ...
    fun `Windows — winget + PowerShell(관리자) + exe`() {
        ...
        assertTrue(s.contains("releases/latest/download/discord-ai-network-bot-windows.exe"), s)
        assertFalse(s.contains("discord-ai.yeon.world/download"), s)
        assertTrue(s.contains("discord-ai-network-bot-windows.exe"), s)
        assertTrue(s.contains(".\\discord-ai-network-bot-windows.exe"), s)
