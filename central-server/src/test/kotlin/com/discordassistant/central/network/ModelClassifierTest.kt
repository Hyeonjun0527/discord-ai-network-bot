...
class ModelClassifierTest {
    ...
    fun `소형 모델은 패밀리와 무관하게 LIGHT`() {
        ...
        assertEquals("LIGHT", ModelClassifier.maxBurden(listOf("qwen3.5:0.8b")))
        assertEquals("LIGHT", ModelClassifier.maxBurden(listOf("llama3.1:8b")))
        assertEquals("LIGHT", ModelClassifier.maxBurden(listOf("gemma4:4b")))
        assertEquals("LIGHT", ModelClassifier.maxBurden(listOf("qwen2.5:1.5b")))
    }

    @Test
    fun `중형은 STANDARD, 대형은 HEAVY`() {
        assertEquals("STANDARD", ModelClassifier.maxBurden(listOf("gemma4:12b")))
        assertEquals("STANDARD", ModelClassifier.maxBurden(listOf("qwen2.5-coder:7b")))
        assertEquals("HEAVY", ModelClassifier.maxBurden(listOf("gemma4:27b")))
        assertEquals("HEAVY", ModelClassifier.maxBurden(listOf("qwen2.5:32b")))
        assertEquals("HEAVY", ModelClassifier.maxBurden(listOf("llama3.1:70b")))
    }
