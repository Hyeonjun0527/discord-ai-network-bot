...
- [ ] 7-1. 허용 채널 추가/제거·판정
- [ ] 7-2. 역할→허용 부담수준·일일 한도 매핑
- [ ] 7-3. 멤버 역할→최대 허용 수준 해석(다중 역할)
- [ ] 7-4. 승인 방식·기본 요청 제한 설정
- [ ] 7-5. 정책 변경 audit
- [ ] 7-6. 단위 테스트
- [ ] 7-7. `gradlew build` + 커밋
...
- [ ] 9-1. 후보 생성
- [ ] 9-2. 부담수준 감당·온라인·idle 필터
- [ ] 9-3. 요청자/채널 허용 필터
- [ ] 9-4. 일일 한도·동시 한도 필터
- [ ] 9-5. 과다처리 쿨다운·요청 크기·실패율 필터
- [ ] 9-6. RESTRICTED 특수 필터
- [ ] 9-7. 후보 0/권한부족 신호
- [ ] 9-8. 단계별 사유 기록
- [ ] 9-9. 단위 테스트(각 단계)
- [ ] 9-10. `gradlew build` + 커밋

## K-차수 10 — 공정성 점수 & Router

- [ ] 10-1. provider_score(적합도+온라인+idle+잔여 − 실패율 − 부하 − heavy낭비)
- [ ] 10-2. light→light·heavy→heavy 우선, heavy 낭비 방지 예외
- [ ] 10-3. 최근 과다처리 가산 감점(공정성)
- [ ] 10-4. 동점 분산·최종 선택+사유
- [ ] 10-5. 순수 함수 단위 테스트
- [ ] 10-6. `gradlew build` + 커밋
