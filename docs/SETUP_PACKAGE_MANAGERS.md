...
함께 **렌더된 매니페스트**(`discord-ai-agent-network-bot.rb`, `.json`, winget `*.yaml`)를 릴리스에 첨부한다.
...
```bash
# 1) tap 저장소 생성(공개)
gh repo create yeon-intergation-platform/homebrew-tap --public -d "Homebrew tap — discord-ai-agent-network-bot"

# 2) 릴리스에서 렌더된 formula 내려받기
gh release download agent-v0.1.1 -R Hyeonjun0527/discord-assistant \
  -p discord-ai-agent-network-bot.rb -D /tmp/tap

# 3) tap 에 커밋
git clone https://github.com/yeon-intergation-platform/homebrew-tap /tmp/homebrew-tap
mkdir -p /tmp/homebrew-tap/Formula
cp /tmp/tap/discord-ai-agent-network-bot.rb /tmp/homebrew-tap/Formula/
cd /tmp/homebrew-tap
git add Formula/discord-ai-agent-network-bot.rb
git commit -m "discord-ai-agent-network-bot 0.1.1"
git push
```
...
brew install discord-ai-agent-network-bot      # brew 가 sha256 자동 검증, 관리자 불필요
```
...
```bash
# 1) bucket 저장소 생성(공개)
gh repo create yeon-intergation-platform/scoop-bucket --public -d "Scoop bucket — discord-ai-agent-network-bot"

# 2) 릴리스에서 렌더된 manifest 내려받기
gh release download agent-v0.1.1 -R Hyeonjun0527/discord-assistant \
  -p discord-ai-agent-network-bot.json -D /tmp/sb
...
cp /tmp/sb/discord-ai-agent-network-bot.json /tmp/scoop-bucket/bucket/
cd /tmp/scoop-bucket
git add bucket/discord-ai-agent-network-bot.json
git commit -m "discord-ai-agent-network-bot 0.1.1"
git push
```
...
scoop install discord-ai-agent-network-bot     # scoop 이 hash 자동 검증, 관리자 불필요
```
...
- macOS: `brew update && brew install yeon-intergation-platform/tap/discord-ai-agent-network-bot && discord-ai-agent-network-bot --version`
- Windows: `scoop install discord-ai-agent-network-bot; discord-ai-agent-network-bot --version`
