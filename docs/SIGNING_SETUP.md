...
4. 검증: macOS `spctl -a -vvv Nexa.app`(accepted)·`xcrun stapler validate Nexa.app`·`xcrun stapler validate nexa-macos.dmg`; Windows `signtool verify /pa nexa-windows.exe`. 실제 브라우저로 받아 경고 확인(mac=무경고, win=평판 쌓이는 중이면 초기 경고 가능).
