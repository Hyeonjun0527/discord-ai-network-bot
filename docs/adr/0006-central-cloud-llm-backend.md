...
### 3. 텍스트 LLM 호출 타임아웃 — fast-or-fail (불변)

`central.cloud.llm-timeout-seconds` 는 **3초**이고, 1회 시도가 3초를 넘으면 취소하고 최대
`llm-max-retries`(2)회만 재요청한다. **이 값은 의도된 설계이며 "호출을 성공시키려고" 올려서는 안 된다.**

- 근거: 니아는 **사람 속도로 반응**해야 한다(장면 참여·직접 호명 응답 모두 즉각성이 핵심). 느린 클라우드
  응답을 오래 기다리면 대화 흐름이 끊기고 "사람다움"이 깨진다. 그래서 **느리면 기다리지 않고 빨리 실패**하고,
  로컬 폴백/명확한 오류 노출로 넘긴다(slow success 보다 fast fail).
- 따라서 "클라우드 AI 오류"가 잦다면 **타임아웃을 늘리는 것은 해결책이 아니다.** 진짜 원인(z.ai 키 무효·잔액
  부족·모델명 오류·네트워크·안전 필터)을 고쳐야 한다. 실패 응답의 사유는 `CloudLlmResponseParser` 가
  추출해 사용자(운영자)에게 카테고리로 노출한다(인증/모델없음/잔액/한도/서버오류).
- 타임아웃을 바꾸려면 이 ADR 을 먼저 갱신한다(설정만 몰래 바꾸지 않는다).
