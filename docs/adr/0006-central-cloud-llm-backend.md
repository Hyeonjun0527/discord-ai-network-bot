...
후보 2개**를 받은 뒤 비밀 유출과 버블 형식 검증을 통과한 후보만 고른다. 말투와 대화 품질은 문자열 패턴으로
차단하지 않고 전체 문맥·페르소나·few-shot을 받은 생성 모델이 판단한다. 사람처럼 표현은 달라지되, 높은 온도로 페르소나를
...
(c) 최근 장면을 포함한 후보 2개 생성을 함께 사용한다. 후보 수가 늘어도 외부 LLM 호출 횟수는 한 번이다.
