...
정상 발화에서는 LLM을 두 번 호출하고, **각 호출이 보는 컨텍스트가 서로 다르다.** Judge나 Speech가 실패하면
해당 단계만 한 번 재시도하므로 실패 복구 때만 호출 수가 늘어난다.
...
flowchart TD
    ...
    GEN --> CHECK["로컬 검증 — 비밀 유출·버블 형식·미완결 응답 검사"]
    CHECK --> PICK["로컬 선택 — uncertainty가 가장 낮은 생존 후보"]
```
...
- 후보를 고르기 위한 별도 AI 호출은 없다. Speech 한 번이 후보를 함께 만들고, 실패 때만 같은 생성을 한 번
  재시도하며 로컬 코드가 안전검사 뒤 하나를 고른다.
...
| 후보/호출 | 행동 1개 / 정상 1회 | 문장 후보 2개 / 정상 1회 |
| 출력 한도 | 판단 JSON에 필요한 범위 | 최대 **1,024 tokens**, bubble당 최대 **280자** |
| 목적 | "말할까?" 한 번의 선택 | "뭐라고?" 실제 문구 생성 |
| 실패 시 | 같은 prompt로 1회 재시도 후 로컬 제한 하강 | deadline 안에서 1회 재시도 후 침묵 |

> 판단·발화·무료질문·관리도구는 OpenAI `gpt-5.6-luna`, reasoning `none`으로 통일한다(근거: ADR 0006).
> 발화 다양성은 temperature가 아니라 최근 장면, 관리형 few-shot, 한 번의 Speech 응답 안에서 만든 여러 후보로 확보한다.
...
sequenceDiagram
    ...
    alt 판단이 SPEAK
        ...
        Bot->>Bot: 로컬 critic 통과 후보 중 하나 선택
