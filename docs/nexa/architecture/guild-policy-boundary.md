...
- **MEMBER 모드의 직접 호명은 강한 장면 증거이지 무조건 발화 계약이 아니다**: @멘션·이름 호명·니아 발화 reply는
  participation judge가 원문 장면에서 수신자·턴 소유·참여 기대를 읽을 때 쓰는 신호다. 단, 더 최신의 정정·철회·수신자
  변경이 있으면 이전 호명보다 우선한다. 예를 들어 니아를 불렀다가 다른 구성원에게 한 말이라고 바로잡은 장면에서는,
...
5. MEMBER 모드에서 직접 호명(멘션/이름 호명)·니아 발화 reply는 judge의 강한 장면 증거이지만, 발화를 강제하지 않는다.
   real-send participation의 의미 판단·안전 차단·실패 뒤에는 legacy 폴백을 하지 않으며, 최신 원문 장면이 침묵을
   자연스럽다고 판단하면 침묵도 올바른 결과다. SHADOW는 이 불변식을 관찰하되 사용자 대화의 기존 응답 계약을 바꾸지 않는다.
