...
상태: **구현 완료, formal private approval artifact·독립 semantic benchmark 대기**
...
```text
formally approved private v2 cards
...
색인 embedding에는 `response_mode`, 최소 일반화 `situation`, `style_signals`, `context_bubbles`만 보낸다. `styleMode`는 embedding 문자열에 반복 주입하지 않고 후보를 같은 enum으로 먼저 제한한다. 그 제한된 후보 안에서만 현재 대화·발화 방향과 카드의 색인 embedding을 cosine으로 비교한다.
`response_bubbles`는 색인 대상이 아니다. 선택된 카드의 말풍선은 같은 Speech provider 요청에만 들어가며, 실행 trace와
metric에는 생략한다.

## 현재 import artifact

로컬 private artifact는 Git ignore 및 권한 보호 상태여야 한다. runtime export는 `CURATION_APPROVED` 카드만
수용한다. exporter는 `curation/approved/`의 v2 카드, `APPROVED` 상태, `USE|STYLE_ONLY` decision, source
manifest hash, 그리고 최신 fresh verifier A~F `PASS`를 모두 fail-closed로 확인한다. preview/month-plan/candidate
카드는 runtime JSONL에 넣지 않는다. export는 카드의 원본 파일 경로, 메시지 ID, 날짜, 원문 line trace를 runtime JSONL에
넣지 않는다.
...
4. formal approval gate를 통과한 private import JSONL은 배포 가능한 보호 경로에만 있고 Git/로그에 없다.
...
3. GitHub Actions의 **central Speech-style RAG import**를 수동 실행한다. 이 workflow는 `production` Environment 승인 뒤,
   private JSONL의 digest·source fingerprint·enum·중복 ID를 확인하고 별도 one-shot 컨테이너에서 import한다.
...
- runtime JSONL exporter: formal approval gate, JSONL digest와 private file mode 검증.
- 실제 OpenAI embedding + H2 암호화 저장 + query + top-2 selection + prompt trace 비노출 통합 테스트.
- `NIA_APPROVED_PRIVATE_RETRIEVAL_EVAL_FILE`이 주어질 때, 독립적으로 작성·라벨링한 30개 이상의 private eval query로 의미 검색을 검증한다. 각 query는 원문 카드의 문장을 그대로 복사하지 않고, 허용되는 카드 ID를 미리 지정한다. 같은 enum만 반환해야 하며 hit@1은 70% 이상, hit@2는 90% 이상이어야 한다. 결과 로그에는 카드 문구·query·원문 trace를 넣지 않는다.
