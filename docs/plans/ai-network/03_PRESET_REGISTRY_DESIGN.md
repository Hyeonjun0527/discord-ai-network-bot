...
- [x] 프리셋에 비밀/민감정보가 포함되지 않는가?
- [x] 게시 revision 이 immutable 인가?
- [x] 가져오기 후 원본 수정이 내 서버에 자동 반영되지 않는가?
- [x] high risk preset 이 승인 없이 적용되지 않는가?
- [x] 좋아요/추천이 남용되지 않는가?
- [x] 신고된 프리셋을 숨기거나 중단할 수 있는가?
- [x] 삭제된 프리셋의 기존 import 기록은 보존되는가?
