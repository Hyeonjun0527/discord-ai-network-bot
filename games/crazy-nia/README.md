...
- **넓은 격자 맵 17×13**: 외곽 고정벽 + 무작위 맵 템플릿의 고정 기둥/벽 + 무작위 파괴블록. 시작 코너·인접칸은 비움.
- **여러 랜덤 맵(5종 템플릿)**: 라운드마다 서버가 템플릿(클래식 격자·열린 통로·십자 광장·대각 기둥·쌍둥이 방)을
  랜덤 선택하고 그 위에 파괴블록을 무작위 배치한다. 서버가 결정한 그리드를 동기 → 같은 방의 모두 같은 맵.
- **부드러운 연속 이동**: 서버가 방향키 입력을 받아 고정 속도로 미끄러지듯 이동(속도 아이템으로 가속).
- **폭탄**: 설치 ~2.2초 뒤 폭발. 십자(+) 사거리, 파괴블록 1칸 파괴 후 정지, 연쇄 폭발.
- **사망**: 폭발 화염 칸의 플레이어 사망. **아이템**: 블록 파괴 시 확률(40%) 드롭(폭탄/사거리/속도+1, 각각 구분되는 스프라이트).
- **사운드**: WebAudio 합성 SFX(에셋 다운로드 없음) — 폭탄 설치/폭발/아이템 획득/사망/라운드 시작·승리. `M` 키로 음소거.
