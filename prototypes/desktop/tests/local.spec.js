...
test('로컬 실행: 고급(외부 ComfyUI·HF) 패널을 열면 폴링 재렌더에도 닫히지 않는다(#233 회귀)', async ({ page }) => {
  await page.goto('/index.html');
  await page.click('.nav-item[data-view="local"]');
  const adv = page.locator('#localRuntimes details.rt-adv:not(.rt-adv--guide)');
  await expect(adv).toHaveCount(1);
  // mock 엔 외부 ComfyUI·HF 설정이 없어 기본은 접힘.
  expect(await adv.evaluate((e) => e.open)).toBe(false);
  // 사용자가 펼친다.
  await adv.locator('summary').click();
  expect(await adv.evaluate((e) => e.open)).toBe(true);
  // 재렌더(폴링 load 와 동일 경로) 강제 — 예전엔 여기서 자동으로 닫혔다(open 이 s.comfyUrl||hfOn 로만 계산돼서).
  await page.click('.nav-item[data-view="models"]');
  await page.click('.nav-item[data-view="local"]');
  expect(await page.locator('#localRuntimes details.rt-adv:not(.rt-adv--guide)').evaluate((e) => e.open)).toBe(true);
