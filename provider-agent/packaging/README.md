...
### A. pip
```bash
pip install nexa-agent   # (배포 시) 또는 로컬: pip install -e .
nexa --token <토큰> --relay-url ws://<서버>:8080/agent
```

### B. 단일 실행파일(PyInstaller)
```bash
cd provider-agent && pip install pyinstaller && pyinstaller packaging/agent.spec
# dist/nexa  (Windows/macOS/Linux 각 플랫폼에서 빌드)
./dist/nexa --token <토큰> --relay-url ws://<서버>:8080/agent
```
...
- Linux: `packaging/systemd/nexa.service` → `systemctl --user`(시스템 유닛 아님).
...
- 설정: `~/.config/nexa/config.json`(시크릿 `0600`).
...
## 첫 실행 / 점검
```bash
nexa --self-test --ollama-url http://localhost:11434   # 연결 없이 Ollama 점검
```
...
- 로컬 빌드 체크섬: `shasum -a 256 dist/nexa > SHA256SUMS`
