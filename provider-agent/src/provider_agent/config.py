...
class AgentConfig:
    ...
    auto_update: bool = True  # 시작 시 새 버전을 확인·알림(기본 ON, 적용은 UI 확인 모달 승인 후)
...
def config_from_args(argv: list[str] | None = None) -> tuple[AgentConfig, bool]:
    ...
    # 클라우드 이미지 백엔드(관리자 키 1개로 서버 전체 무료 이미지 — 클라우드 GLM 텍스트와 같은 모델).
    # env > 저장 설정. 키는 이 PC 에만 저장하고 central 엔 올리지 않는다(Ollama 토큰·GLM 키와 동일 원칙).
