...
- macOS: 현재 실행 중인 `.app` 번들을 `/Applications/냥시스턴트.app` 으로 복사(ditto, 서명 보존)
...
def _install_macos() -> dict:
    ...
    return {"ok": True, "target": str(target),
            "message": "응용 프로그램에 추가했어요. Launchpad·Spotlight 에서 ‘냥시스턴트’로 열 수 있어요."}
...
def _install_windows() -> dict:
    ...
    return {"ok": True, "target": str(link),
            "message": "시작 메뉴에 ‘냥시스턴트’를 추가했어요. 시작 메뉴에서 검색해 열 수 있어요."}
