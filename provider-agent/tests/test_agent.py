...
def test_image_capability_requires_ready_image_backend():
    """이미지 provider 광고는 SD 준비 상태에서 켜진다. 안전 심사와 번역은 central이 담당한다."""
...
@pytest.mark.asyncio
async def test_image_without_central_pretranslation_is_blocked_before_sd():
    """직접 image frame 이 들어와도 central 안전 심사 증표가 없으면 fail-closed."""
    class CountingSD:
        calls = 0
        checkpoint_calls = 0
    ...
    agent._image_ready = True
    conn = FakeConn()
    await agent._run_infer(conn, InferRequest(request_id="img-no-proof", prompt="고양이", task="image"), model=None)

    assert sd.calls == 0
    assert isinstance(conn.sent[0], InferError)
    assert "중앙 서버의 이미지 안전 심사" in conn.sent[0].message
...
async def test_handle_image_emits_progress_then_data():
    ...
    완료 시 b64 데이터 청크 + done 을 보낸다(외부 LLM 호출 없음)."""
    from provider_agent.protocol import ChunkFrame
...
async def test_set_image_enabled_toggles_capability():
    """이미지 capability 는 SD 준비 상태와 함께 켜지고 꺼진다."""
