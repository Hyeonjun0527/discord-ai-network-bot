...
def test_cloud_image_still_gated_by_gemini_safety():
    # 클라우드 백엔드라도 provider-local Gemini 안전심사가 없으면 이미지 capability 를 광고하지 않는다.
    agent = ProviderAgent(
        AgentConfig(token="T", image_backend="stability", stability_api_key="sk"), ollama=object()
    )
    agent._image_ready = True
    assert agent._image_for(100) is False  # Gemini(안전심사) 없으면 차단
...
def _isolate_config(monkeypatch):
    ...
    for k in ("STABILITY_API_KEY", "RUNPOD_API_KEY", "RUNPOD_ENDPOINT_ID", "IMAGE_BACKEND", "GEMINI_API_KEY"):
