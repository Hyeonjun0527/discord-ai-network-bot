...
def test_macos_install_copies_bundle_to_applications(monkeypatch, tmp_path):
    ...
    (bundle / "Contents" / "MacOS" / "냥시스턴트").write_text("bin")
    target = tmp_path / "Applications" / "냥시스턴트.app"
    target.parent.mkdir()
    ...
    r = installer.install_app()
    assert r["ok"] is True
    assert (target / "Contents" / "MacOS" / "냥시스턴트").read_text() == "bin"
...
def test_windows_install_copies_and_makes_shortcut(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kw):
        ran["cmd"] = cmd
        Path(startmenu / "냥시스턴트.lnk").write_text("lnk")  # PowerShell 이 만들 .lnk 흉내
    ...
    r = installer.install_app()
    assert r["ok"] is True
    assert (programs / "냥시스턴트.exe").read_text() == "exe"  # 사용자 폴더로 복사
    assert "powershell" in ran["cmd"][0]  # 바로가기 생성에 PowerShell 사용
    assert (startmenu / "냥시스턴트.lnk").exists()
