...
def exact_matches(query: str, limit: int) -> list[dict]:
    ...
    for chunk_id, score in sorted(scores.items(), key=lambda item: item[1], reverse=True)[:limit]:
        row = conn.execute(
            "SELECT id, source_file, chunk_type, title, content FROM chunks WHERE id=?",
            (chunk_id,),
        ).fetchone()
        if row:
            result.append(to_result(row, score + 1000))
    conn.close()
    return result


def keyword_matches(query: str, limit: int) -> list[dict]:
...
def search(query: str, limit: int) -> list[dict]:
    ...
    for item in exact_matches(query, limit * 2) + keyword_matches(query, limit * 2):
...
def main() -> None:
    ...
    for idx, item in enumerate(search(args.query, args.limit), 1):
        print("─" * 64)
